...
gBellPepperInfo.SelectDom = function (dom1) { var x1, y1; var rec1, mg = 0; if (dom1.parentNode.nodeName == 'BODY') { bell_pepper_info.Log('select dom: body'); return; } rec1 = this.GetLoc(dom1); this.ShowMark(window.scrollX + rec1.left - 4, window.scrollY + rec1.top - 4, rec1.width + 4, rec1.height + 4); x1 = window.scrollX; y1 = window.scrollY; if (window.innerWidth < rec1.right + mg) { x1 = Math.max(0, window.scrollX + rec1.right + mg - window.innerWidth); } else if (rec1.left < 0) { x1 = Math.max(0, window.scrollX + rec1.left); } if (window.innerHeight < rec1.bottom + mg) { y1 = Math.max(0, window.scrollY + rec1.bottom + mg - window.innerHeight); } else if (rec1.top < 0) { y1 = Math.max(0, window.scrollY + rec1.top); } if (x1 != window.scrollX || y1 != window.scrollY) { window.scroll(x1, y1); } else { } };
...
gBellPepperInfo.ChkDom = function (dom1) { var dom2; for (dom2 = dom1; dom2 != null; dom2 = dom2.parentNode) { if (dom2 == document.body) { return true; } } return false; };
...
gBellPepperInfo.OnTouchStartFnc = function (ev) { var touch = ev.touches[0]; this.TouchCnt = ev.touches.length; this.TouchMoved = false; this.TouchStartX = touch.pageX; this.TouchStartY = touch.pageY; };
gBellPepperInfo.OnTouchMoveFnc = function (ev) { var touch = ev.touches[0]; this.TouchMoved = true; };
gBellPepperInfo.OnTouchEndFnc = function (ev) { var dom1, rec1; var rng1, dom2, dom3, x1, y1, x2, y2, time1, children; try { if (this.TouchCnt == 1 && !this.TouchMoved) { dom3 = null; dom1 = ev.target; if (dom1.hasChildNodes()) { rng1 = document.createRange(); time1 = (new Date()).getTime(); children = dom1.childNodes; for (var i = 0; i < children.length; i++) { if (i % 20 == 0 && 3000 < (new Date()).getTime() - time1) { bell_pepper_info.Log('time out'); break; } dom2 = children[i]; rng1.selectNode(dom2); rec1 = rng1.getBoundingClientRect(); if (rec1 == null) { } else { x1 = window.scrollX + rec1.left; y1 = window.scrollY + rec1.top; x2 = x1 + rec1.width; y2 = y1 + rec1.height; if (x1 <= this.TouchStartX && this.TouchStartX <= x2 && y1 <= this.TouchStartY && this.TouchStartY <= y2) { dom3 = dom2; break; } else { } } } } if (dom3 == null) { dom1 = this.FirstDom(ev.target); } else { dom1 = this.FirstDom(dom3); } if (dom1 != null) { switch (dom1.nodeName) { case '#text': case 'SELECT': case 'RUBY': this.CurDom = dom1; rec1 = this.GetLoc(dom1); this.ShowMark(window.scrollX + rec1.left - 4, window.scrollY + rec1.top - 4, rec1.width + 4, rec1.height + 4); break; } } } } catch (e) { bell_pepper_info.Log('Touch end err:' + e.message); } this.TouchCnt = 0; this.TouchMoved = false; };
gBellPepperInfo.PageFinished = function () { if (document.m_BellPepperInfo == undefined) { document.m_BellPepperInfo = gBellPepperInfo; bell_pepper_info.Log('Page loading completed'); document.addEventListener('touchstart', function (ev) { gBellPepperInfo.OnTouchStartFnc(ev); }, false); document.addEventListener('touchmove', function (ev) { gBellPepperInfo.OnTouchMoveFnc(ev); }, false); document.addEventListener('touchend', function (ev) { gBellPepperInfo.OnTouchEndFnc(ev); }, false); } else { bell_pepper_info.Log('Page initialized'); } };
gBellPepperInfo.SpeakCur = function () { this.StopTimer(); this.PageFinished(); if (this.CurDom == null || !this.ChkDom(this.CurDom)) { this.CurDom = this.FirstDom(document.body.firstChild); } if (this.CurDom == null) { bell_pepper_info.Speak(0, 'empty document'); } else { if (bell_pepper_info.ReadPhrase() == 2) { this.SpeakDom2(null, this.CurDom); } else { this.SpeakDom(this.CurDom); } } };
